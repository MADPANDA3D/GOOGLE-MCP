...
MCP_MODE = os.getenv("MCP_MODE", "standalone").strip().lower()
MCP_ACCESS_TOKEN = os.getenv("MCP_ACCESS_TOKEN", "")
MCP_REQUEST_BODY_MAX_BYTES = max(int(os.getenv("MCP_REQUEST_BODY_MAX_BYTES", "131072")), 1024)
MCP_PROVIDER_RESPONSE_MAX_BYTES = max(
    int(os.getenv("MCP_PROVIDER_RESPONSE_MAX_BYTES", "4194304")),
    4096,
)
MCP_TOOL_OUTPUT_MAX_BYTES = max(int(os.getenv("MCP_TOOL_OUTPUT_MAX_BYTES", "1048576")), 4096)
MCP_PROVIDER_ERROR_MAX_CHARS = max(int(os.getenv("MCP_PROVIDER_ERROR_MAX_CHARS", "1024")), 128)
...
MCP_ALLOWED_ORIGINS = tuple(
    origin.strip() for origin in os.getenv("MCP_ALLOWED_ORIGINS", "").split(",") if origin.strip()
)
GOOGLE_TOKEN_URI = "https://oauth2.googleapis.com/token"
GOOGLE_CREDENTIALS_PATH = os.getenv("GOOGLE_CREDENTIALS_PATH", "fastmcp/.google/credentials.json")
...
MCP_MAX_BATCH_ITEMS = max(1, min(int(os.getenv("MCP_MAX_BATCH_ITEMS", "1000")), 10000))
MCP_MAX_PROVIDER_CALLS_PER_TOOL = max(
    1,
    min(int(os.getenv("MCP_MAX_PROVIDER_CALLS_PER_TOOL", "128")), 1000),
)
...
MCP_IMAGE_REFERENCE = os.getenv("MCP_IMAGE_REFERENCE", "development").strip() or "development"
MCP_STRICT_PARAMS = os.getenv("MCP_STRICT_PARAMS", "").lower() in {"1", "true", "yes"}
MCP_ALLOW_REQUEST_OVERRIDES = os.getenv("MCP_ALLOW_REQUEST_OVERRIDES", "true").lower() in {
    "1",
    "true",
    "yes",
}
...
MCP_GOOGLE_CLIENT_ID_HEADER = (
    os.getenv("MCP_GOOGLE_CLIENT_ID_HEADER", "x-google-client-id").strip().lower()
)
MCP_GOOGLE_CLIENT_SECRET_HEADER = (
    os.getenv("MCP_GOOGLE_CLIENT_SECRET_HEADER", "x-google-client-secret").strip().lower()
)
MCP_GOOGLE_REFRESH_TOKEN_HEADER = (
    os.getenv("MCP_GOOGLE_REFRESH_TOKEN_HEADER", "x-google-refresh-token").strip().lower()
)
MCP_GOOGLE_MAPS_API_KEY_HEADER = (
    os.getenv("MCP_GOOGLE_MAPS_API_KEY_HEADER", "x-google-maps-api-key").strip().lower()
)
MCP_PORTAL_GRANT_HEADER = (
    os.getenv("MCP_PORTAL_GRANT_HEADER", "x-madpanda-portal-grant").strip().lower()
)
MCP_PORTAL_GRANT_TOKEN = os.getenv("MCP_PORTAL_GRANT_TOKEN", "")
GOOGLE_MAPS_API_KEY = os.getenv("GOOGLE_MAPS_API_KEY", "")
MCP_BYOK_CLIENT_CACHE_SIZE = max(int(os.getenv("MCP_BYOK_CLIENT_CACHE_SIZE", "0")), 0)
MCP_BYOK_CLIENT_CACHE_TTL_SECONDS = max(
    float(os.getenv("MCP_BYOK_CLIENT_CACHE_TTL_SECONDS", "0")),
    0.0,
)
...
def _write_private_text(path: str, value: str) -> None:
    ...
    descriptor, temporary = tempfile.mkstemp(prefix=".google-token-", dir=directory, text=True)
    try:
...
class GoogleWorkspaceClient:
    ...
    def _save_token(self, creds: Credentials) -> None:
        ...
        _write_private_text(self.token_path, creds.to_json())
                ...
                if self.authorized_user_info:
                    ...
                    if not self.token_path or not os.path.exists(self.token_path):
                        raise FileNotFoundError(
                            "Missing configured fallback token file. Public profiles require "
                            "request-scoped Google BYOK headers; use file fallback only in an "
                            "explicitly opted-in private deployment."
                        )
                    self._creds = Credentials.from_authorized_user_file(self.token_path)
            creds = self._creds
            if not creds.valid:
...
ACTIVE_GOOGLE_CLIENT: contextvars.ContextVar[GoogleWorkspaceClient | None] = contextvars.ContextVar(
    "active_google_client", default=None
)
...
def _validate_raw_request_url(url: str) -> str:
    ...
    if not cleaned.startswith(("http://", "https://")):
        if not cleaned.startswith("/"):
            cleaned = "/" + cleaned
        cleaned = f"https://www.googleapis.com{cleaned}"

    parsed = urllib.parse.urlparse(cleaned)
    if parsed.scheme.lower() != "https":
        raise ValueError("raw_request requires HTTPS for every provider request.")
    if parsed.username is not None or parsed.password is not None:
        raise ValueError("raw_request url cannot contain user information.")
    try:
        port = parsed.port
    except ValueError as exc:
        raise ValueError("raw_request url contains an invalid port.") from exc
    if port not in {None, 443}:
        raise ValueError("raw_request only permits the standard HTTPS port.")
    host = (parsed.hostname or "").lower()
    if host not in allowed_hosts:
        raise ValueError(
            "raw_request url host is not allowed. "
            "Use an approved Google API host or a relative path like /drive/v3/files."
        )
    try:
        ...
        raise ValueError("raw_request url query is invalid or too large.") from exc
    _validate_raw_request_params({key: value for key, value in query_items})
    return cleaned


def _validate_raw_request_params(params: dict[str, Any] | None) -> None:
    if not params:
        return
    ...
    unsafe = [
        ...
        or str(key).lower() == "key"
    ]
    if unsafe:
        raise ValueError(
            "raw_request params cannot contain credential-bearing keys: "
            + ", ".join(sorted(unsafe))
            + "."
        )
...
def _bounded_provider_payload(
    ...
    *,
    ...
    """Read and decode a direct provider response without unbounded buffering."""
    response_limit = MCP_PROVIDER_RESPONSE_MAX_BYTES
    if max_bytes is not None:
        try:
            requested_limit = int(max_bytes)
        except (TypeError, ValueError) as exc:
            raise ValueError("Provider response limit must be a positive integer.") from exc
        if requested_limit <= 0:
            raise ValueError("Provider response limit must be a positive integer.")
        response_limit = min(response_limit, requested_limit)
    ...
    if content_length:
        ...
        if declared_length is not None and declared_length > response_limit:
            ...
            raise GoogleProviderError(
                f"{provider} response exceeded the configured provider-response limit.",
                error_type="bounded_response",
                status=status or None,
                action="Narrow the provider request and try again.",
            )
        ...
        if not body.strip():
            ...
            try:
                ...
                raise GoogleProviderError(
                    ...
                    status=status or None,
                    action="Retry the provider request or narrow its response fields.",
                ) from exc
    elif body:
        payload["text"] = _clip_string(
            body.decode("utf-8", errors="replace"),
            max_chars=max(MCP_TOOL_OUTPUT_MAX_BYTES // 2, 1024),
        )
    else:
        payload["body_empty"] = True
    return payload
...
def _safe_error_message(value: Any) -> str:
    ...
    credential_key = (
        ...
        r"api[_-]?key|password|credential)"
    )
    text = re.sub(
        r"(?i)(\bauthorization\s*[:=]\s*bearer\s+|\bbearer\s+)[^\s,;}]+",
        r"\1[REDACTED]",
        text,
    )
    text = re.sub(
        rf"(?i)([\"']?{credential_key}[\"']?\s*[:=]\s*)([\"'])(.*?)(\2)",
    ...
    text = re.sub(
        r"(?i)([?&](?:key|access_token|refresh_token|client_secret)=)[^&\s]+",
        r"\1[REDACTED]",
        text,
    )
    if len(text) > MCP_PROVIDER_ERROR_MAX_CHARS:
        text = text[: MCP_PROVIDER_ERROR_MAX_CHARS - 3].rstrip() + "..."
    return text
...
def _classify_error(exc: Exception) -> dict[str, Any]:
    ...
    if isinstance(exc, GoogleProviderError):
        return {
            ...
            "action": _safe_error_message(exc.action),
        }
    if isinstance(exc, RefreshError):
        return {
            "type": "auth_error",
            "message": "Google OAuth credential refresh failed.",
            "action": "Verify the request-scoped Google OAuth credentials.",
        }
    if isinstance(exc, HttpError):
        ...
        return {
            "type": error_type,
            "message": f"Google API request failed with HTTP status {status or 'unknown'}.",
            "status": status,
    ...
    return {
        "type": "unknown_error",
        "message": _safe_error_message(exc) or "Tool execution failed.",
...
def _response_payload(
    ...
    initial_raw = json_dumps(payload)
    initial_bytes = len(initial_raw.encode("utf-8"))
    if initial_bytes > MCP_TOOL_OUTPUT_MAX_BYTES:
        payload["data"] = (
            {
                "truncated": True,
                "original_bytes": initial_bytes,
                "message": "Tool output exceeded the configured response limit.",
            }
            if ok
            else None
        )
        if not ok:
            payload["error"] = {
                "type": "bounded_error",
                "message": "Provider error exceeded the configured response limit.",
                "action": "Narrow the request and try again.",
            }
        payload["meta"]["truncated"] = True
        payload["meta"]["original_bytes_out"] = initial_bytes
    last_len = -1
    raw = ""
    for _ in range(3):
        ...
        last_len = raw_len
    payload["meta"]["serialization_ms"] = round((time.perf_counter() - start) * 1000, 2)
    raw = json_dumps(payload)
    payload["meta"]["bytes_out"] = len(raw.encode("utf-8"))
    return json_dumps(payload)


def _bounded_json_dumps(data: Any) -> str:
    raw = json_dumps(data)
    raw_bytes = len(raw.encode("utf-8"))
    if raw_bytes <= MCP_TOOL_OUTPUT_MAX_BYTES:
        return raw
    return json_dumps(
        {
            "truncated": True,
    ...
    *,
    ...
    while True:
        try:
            ...
            return _bounded_json_dumps(result)
            ...
            if MCP_LOG_REQUESTS:
                logger.warning(
                    "tool_error request_id=%s api=%s action=%s retries=%s error_type=%s status=%s",
                    request_id,
                    api,
                    action,
                    retries,
            ...
            return _bounded_json_dumps({"error": last_error, "meta": meta})
...
def _server_meta() -> dict[str, Any]:
    return {
        ...
        "source_fingerprint": MCP_SOURCE_FINGERPRINT,
        "image_reference": MCP_IMAGE_REFERENCE,
    }
    ...
    if requested_email:
        selected = next(
            (
                ...
                if str(entry.get("sendAsEmail", "")).strip().lower() == requested_email
            ),
            None,
        )
        if selected is None:
...
def _gmail_html_contains_signature(body_html: str, signature_html: str) -> bool:
    ...
    signature_block = (
        f'<div {GMAIL_SIGNATURE_MARKER}="{fingerprint[:16]}">{normalized_signature}</div>'
    )
    suffix = re.sub(
        r"(?:</(?:body|html)>\s*)+$",
        "",
...
def _gmail_plain_contains_signature(body_text: str, signature_html: str) -> bool:
    signature_text = _normalize_signature_text(_gmail_signature_plain_text(signature_html))
    return bool(
        signature_text
        and _normalize_signature_text(_gmail_current_plain_text(body_text)).endswith(signature_text)
    )


def _gmail_signature_html_block(signature: GmailSendAsSignature) -> str:
    return f'<div {GMAIL_SIGNATURE_MARKER}="{signature.fingerprint[:16]}">{signature.html}</div>'
...
def _decode_email_bytes(raw_base64: str) -> bytes:
    try:
        ...
        raise ValueError("raw_base64 must be a valid base64url MIME message.") from exc


def _decode_email_message(raw_base64: str) -> EmailMessage:
    try:
        parsed = BytesParser(policy=policy.default).parsebytes(_decode_email_bytes(raw_base64))
    except (KeyError, LookupError, UnicodeDecodeError, ValueError) as exc:
        raise ValueError("raw_base64 must contain a valid MIME message.") from exc
...
def _verify_gmail_message_fingerprint(
    ...
    if not hmac.compare_digest(
        ...
        raise ValueError(
            "The Gmail draft changed after preview. Preview the final draft again before sending."
        )
...
def _effective_download_limit(requested: int) -> int:
    try:
        value = int(requested)
    except (TypeError, ValueError) as exc:
    ...
    return min(value, DEFAULT_MAX_DOWNLOAD_BYTES)


def _validate_fanout_items(
    values: list[Any] | None,
    argument_name: str,
    *,
    items_per_provider_call: int = 1,
) -> None:
    count = len(values or [])
    provider_batch_size = max(1, int(items_per_provider_call))
    effective_limit = min(
        MCP_MAX_BATCH_ITEMS,
        MCP_MAX_PROVIDER_CALLS_PER_TOOL * provider_batch_size,
    )
    if count > effective_limit:
        raise ValueError(
            f"{argument_name} exceeds the {effective_limit}-item execution limit "
            "for one tool call. Split the work into smaller requests."
        )


def _validate_gmail_scan_fanout(max_messages: int, page_size: int) -> tuple[int, int]:
    effective_messages = _clamp_int(max_messages, minimum=1, maximum=MCP_MAX_BATCH_ITEMS)
    effective_page_size = _clamp_int(page_size, minimum=1, maximum=500)
    list_calls = (effective_messages + effective_page_size - 1) // effective_page_size
    metadata_calls = (effective_messages + GMAIL_METADATA_BATCH_SIZE - 1) // (
        GMAIL_METADATA_BATCH_SIZE
    )
    estimated_calls = list_calls + metadata_calls
    if estimated_calls > MCP_MAX_PROVIDER_CALLS_PER_TOOL:
        raise ValueError(
            "max_messages and page_size would exceed the per-tool provider-call limit. "
            "Reduce max_messages or increase page_size."
        )
    return effective_messages, effective_page_size
...
def _require_maps_api_key() -> str:
    ...
    api_key = headers.get(MCP_GOOGLE_MAPS_API_KEY_HEADER, "")
    if not api_key and not MCP_DISABLE_DEFAULT_GOOGLE_FALLBACK:
        api_key = GOOGLE_MAPS_API_KEY
    if not api_key:
        message = (
            f"Missing Google Maps API key. Provide {MCP_GOOGLE_MAPS_API_KEY_HEADER} on the request."
        )
        if not MCP_DISABLE_DEFAULT_GOOGLE_FALLBACK:
            message += " GOOGLE_MAPS_API_KEY is also accepted because fallback mode is enabled."
...
def _provider_error(
    ...
    *,
    ...
    error_payload = (
        payload.get("json", {}).get("error") if isinstance(payload.get("json"), dict) else None
    )
    provider_status = ""
    if isinstance(payload.get("json"), dict):
        provider_status = str(payload["json"].get("status") or "")
    message = message or str(
        (error_payload or {}).get("message") or payload.get("text") or f"{provider} request failed"
    )
    error_type = "api_error"
    action = "Review the Google API error and adjust the request."
...
def _maps_request(
    ...
    *,
    ...
    payload = _bounded_provider_payload(response, "Maps")
    if not response.ok:
...
def _google_json_request(
    ...
    *,
    ...
    payload = _bounded_provider_payload(response, "Google API")
    if not response.ok:
        raise _provider_error(
            "Google API",
            payload,
...
BILLABLE_READ_GOOGLE_TOOL = ToolAnnotations(
    readOnlyHint=True,
    destructiveHint=True,
    idempotentHint=False,
    openWorldHint=True,
...
TOOL_DESCRIPTIONS = {
    "google_raw_request": (
        "Use this hidden read-only Google API escape hatch only when a documented GET "
        "or HEAD endpoint is not covered by a curated tool. Hosts, parameters, headers, "
        "methods, and returned data are constrained and bounded."
    ),
    "drive_list_files": (
    ...
    "check_configuration": (
        "Use this local read-only navigation tool to check service-access readiness and "
...
COMMON_PARAMETER_DESCRIPTIONS = {
    ...
    "mode": "Deletion mode.",
    "confirm": "Required true for destructive operations and billable Google Maps calls.",
    ...
    "headers": "Deprecated raw-request argument; caller-supplied raw HTTP headers are rejected.",
    ...
    "json_body": "Deprecated raw-request argument; raw request bodies are rejected.",
...
def _annotation_for_tool(tool_name: str) -> ToolAnnotations:
    ...
    if tool_name in BILLABLE_MAPS_TOOLS:
        return BILLABLE_READ_GOOGLE_TOOL
...
async def google_raw_request(
    ...
    """Send a constrained read-only request to an allowlisted Google API host."""
    normalized_method = method.strip().upper()
    if normalized_method not in {"GET", "HEAD"}:
        raise ValueError("raw_request only permits GET and HEAD methods.")
    if json_body is not None:
        raise ValueError("raw_request does not permit a JSON request body.")
    if headers:
        raise ValueError("raw_request does not permit caller-supplied HTTP headers.")
    _validate_raw_request_params(params)
    ...
    def _request():
        ...
        payload = _bounded_provider_payload(response, "Google raw request")
...
async def drive_batch_get_metadata(
    ...
    def _batch_get():
        ...
        _validate_fanout_items(file_ids, "file_ids")
        service, cached = client.get_service("drive", "v3")
        files = []
...
async def drive_upload_file(
    ...
    def _upload():
        ...
        if mode == "resumable":
            ...
            response_payload = _bounded_provider_payload(response, "Google Drive")
            payload: dict[str, Any] = {
                **response_payload,
                "headers": {
        ...
        service, cached = client.get_service("drive", "v3")
        data = base64.b64decode(content.encode("ascii")) if is_base64 else content.encode("utf-8")
...
async def drive_download_file(
    ...
    def _download():
        if not file_id:
            raise ValueError("file_id cannot be empty")
        effective_max_bytes = _effective_download_limit(max_bytes)
        ...
        if size is not None:
            try:
                ...
                if size_int > effective_max_bytes:
                    response_payload["too_large"] = True
                    response_payload["size"] = size_int
                    response_payload["max_bytes"] = effective_max_bytes
                    return response_payload, {"cached_service": cached_service}
        ...
        if not response.ok:
            raise RuntimeError(f"Drive download failed with status {response.status_code}.")
...
async def drive_list_permissions(
    file_id: str, fields: str = "permissions(id,type,role,emailAddress,domain),nextPageToken"
) -> str:
...
@mcp.tool()
async def drive_list_comments(
    file_id: str,
    fields: str = "comments(id,content,author,createdTime,modifiedTime,resolved),nextPageToken",
) -> str:
...
async def drive_create_comment(file_id: str, content: str, confirm: bool = False) -> str:
    ...
    def _create_comment():
        ...
        request = service.comments().create(
            fileId=file_id, body={"content": content}, fields="id,content"
        )
        return request.execute(), {"cached_service": cached}
...
@mcp.tool()
async def drive_update_comment(
    file_id: str, comment_id: str, content: str, confirm: bool = False
) -> str:
...
async def drive_delete_comment(file_id: str, comment_id: str, confirm: bool = False) -> str:
    ...
    def _delete_comment():
        ...
        return {"file_id": file_id, "comment_id": comment_id, "deleted": True}, {
            "cached_service": cached
        }
...
async def drive_list_revisions(
    file_id: str, fields: str = "revisions(id,mimeType,modifiedTime,keepForever),nextPageToken"
) -> str:
...
async def drive_update_file_metadata(
    file_id: str, metadata: dict[str, Any], confirm: bool = False
) -> str:
    ...
    def _update_file_metadata():
        ...
        service, cached = client.get_service("drive", "v3")
        request = service.files().update(
            fileId=file_id, body=metadata, fields=DEFAULT_DRIVE_GET_FIELDS
        )
        return request.execute(), {"cached_service": cached}
...
async def docs_replace_text(
    ...
    def _replace():
        _require_confirm("replace text in a Google Doc", confirm)
        if not document_id:
            raise ValueError("document_id cannot be empty")
...
async def sheets_get_spreadsheet(spreadsheet_id: str, fields: str = "") -> str:
    ...
    def _get_sheet():
        ...
        request = service.spreadsheets().get(spreadsheetId=spreadsheet_id, fields=effective_fields)
        return request.execute(), {"cached_service": cached}
...
async def sheets_update_values(
    ...
    def _update_values():
        _require_confirm("update Google Sheet values", confirm)
        if not spreadsheet_id:
            raise ValueError("spreadsheet_id cannot be empty")
        if not range_a1:
            raise ValueError("range_a1 cannot be empty")
...
async def sheets_append_values(
    ...
    def _append_values():
        _require_confirm("append Google Sheet values", confirm)
...
async def sheets_batch_update_values(
    ...
    return await run_tool("sheets", "batch_update_values", _batch_update_values, allow_retry=False)
...
async def sheets_batch_clear_values(
    ...
    return await run_tool("sheets", "batch_clear_values", _batch_clear_values, allow_retry=False)
...
async def sheets_get_by_data_filter(spreadsheet_id: str, data_filters: list[dict[str, Any]]) -> str:
    ...
    return await run_tool("sheets", "get_by_data_filter", _get_by_data_filter, allow_retry=True)
...
async def slides_create_presentation(title: str) -> str:
    ...
    return await run_tool("slides", "create_presentation", _create_presentation, allow_retry=False)
...
async def slides_replace_text(
    ...
    def _replace():
        _require_confirm("replace text in a Google Slides presentation", confirm)
        if not presentation_id:
            raise ValueError("presentation_id cannot be empty")
        if not contains_text:
            raise ValueError("contains_text cannot be empty")
...
async def slides_get_page_thumbnail(
    ...
    return await run_tool("slides", "get_page_thumbnail", _get_page_thumbnail, allow_retry=True)
...
@mcp.tool()
async def gmail_get_message_headers(message_id: str, headers: list[str] | None = None) -> str:
    ...
    def _get_headers():
        ...
        header_map = {
            entry.get("name"): entry.get("value") for entry in header_list if entry.get("name")
        }
        return (
            {
                "id": data.get("id"),
...
async def gmail_batch_get_metadata(
    ...
    def _batch_get():
        ...
        _validate_fanout_items(message_ids, "message_ids")
        service, cached = client.get_service("gmail", "v1")
        effective_headers = metadata_headers or list(DEFAULT_GMAIL_METADATA_HEADERS)
        results = []
...
async def gmail_signature_preflight(
    ...
    def _preflight():
        ...
        if parsed_message is not None and not _gmail_message_contains_signature(
            ...
            raise ValueError("The Gmail message is missing the configured send-as signature.")
        return {
            "signature_alias": signature.alias,
            "signature_fingerprint": signature.fingerprint,
            "message_fingerprint": (
                _gmail_raw_message_fingerprint(effective_raw) if effective_raw else ""
            ),
        }, {
            "cached_service": cached,
...
async def gmail_send_message(
    ...
    def _send():
        _require_confirm("send a Gmail message", confirm)
        if not to:
...
async def gmail_send_raw_message(
    ...
    def _send_raw():
        _require_confirm("send a raw Gmail message", confirm)
        if not raw_base64:
...
async def gmail_send_draft(
    ...
    def _send_draft():
        _require_confirm("send a Gmail draft", confirm)
        if not draft_id:
...
async def gmail_modify_message_labels(
    ...
    def _modify():
        _require_confirm("modify Gmail message labels", confirm)
        if not message_id:
            raise ValueError("message_id cannot be empty")
        service, cached = client.get_service("gmail", "v1")
        body = {
...
async def gmail_update_draft(
    ...
    def _update_draft():
        ...
        if not subject:
            raise ValueError("subject cannot be empty")
        _require_confirm("replace a Gmail draft", confirm)
        service, cached = client.get_service("gmail", "v1")
        signature = _resolve_gmail_send_as_signature(service, from_alias)
        _verify_gmail_signature_fingerprint(
            signature,
...
async def gmail_get_attachment(
    ...
    def _get_attachment():
        ...
        metadata_response = session.request(
            "GET",
            url,
            params={"fields": "size"},
            stream=True,
        ...
        metadata_payload = _bounded_provider_payload(
            metadata_response,
        ...
        if not metadata_response.ok:
            raise _provider_error(
                ...
                retry_after=_retry_after_seconds(dict(metadata_response.headers or {})),
            )
        metadata = metadata_payload.get("json")
        if not isinstance(metadata, dict):
            raise GoogleProviderError(
                "Gmail returned invalid attachment metadata.",
                error_type="invalid_provider_response",
                status=metadata_response.status_code,
        ...
        encoded_limit = ((effective_max_bytes + 2) // 3) * 4
        content_response_limit = min(
            MCP_PROVIDER_RESPONSE_MAX_BYTES,
            encoded_limit + 4096,
        )
        content_response = session.request(
            "GET",
            url,
        ...
        content_payload = _bounded_provider_payload(
            content_response,
            "Gmail attachment content",
            max_bytes=content_response_limit,
        )
        if not content_response.ok:
            raise _provider_error(
                ...
                retry_after=_retry_after_seconds(dict(content_response.headers or {})),
            )
        content = content_payload.get("json")
...
async def gmail_batch_modify_messages(
    ...
    def _batch_modify():
        ...
        if not ids:
            raise ValueError("message_ids cannot be empty")
        _validate_fanout_items(ids, "message_ids", items_per_provider_call=1000)
...
async def gmail_sender_clusters(
    ...
    def _sender_clusters():
        effective_max_messages, effective_page_size = _validate_gmail_scan_fanout(
            max_messages,
            page_size,
        )
        service, cached = client.get_service("gmail", "v1")
...
async def gmail_apply_cleanup_plan(
    ...
    def _apply_plan():
        ...
        _validate_fanout_items(
            ids,
            "message_ids",
            items_per_provider_call=1 if normalized_action == "trash" else 1000,
        )
...
async def calendar_batch_get_events(
    ...
    def _batch_get():
        ...
        _validate_fanout_items(event_ids, "event_ids")
        service, cached = client.get_service("calendar", "v3")
...
async def calendar_update_event(
    ...
    def _update_event():
        _require_confirm("update a Google Calendar event", confirm)
        if not calendar_id:
            raise ValueError("calendar_id cannot be empty")
        if not event_id:
            raise ValueError("event_id cannot be empty")
...
async def calendar_move_event(
    ...
    def _move_event():
        if not source_calendar_id or not destination_calendar_id or not event_id:
            raise ValueError(
                "source_calendar_id, destination_calendar_id, and event_id are required"
            )
...
@mcp.tool()
async def calendar_import_event(
    calendar_id: str, event_body: dict[str, Any], confirm: bool = False
) -> str:
...
async def calendar_get_acl(calendar_id: str, rule_id: str) -> str:
    ...
    def _get_acl():
        ...
        service, cached = client.get_service("calendar", "v3")
        return service.acl().get(calendarId=calendar_id, ruleId=rule_id).execute(), {
            "cached_service": cached
        }
...
async def calendar_delete_acl(calendar_id: str, rule_id: str, confirm: bool = False) -> str:
    ...
    def _delete_acl():
        ...
        return {"calendar_id": calendar_id, "rule_id": rule_id, "deleted": True}, {
            "cached_service": cached
        }
...
async def youtube_list_playlist_items(
    ...
    return await run_tool("youtube", "list_playlist_items", _list_playlist_items, allow_retry=True)
...
@mcp.tool()
async def searchconsole_inspect_url(
...
async def business_profile_list_locations(
    ...
    return await run_tool("business_profile", "list_locations", _list_locations, allow_retry=True)
...
async def maps_geocode(
    ...
    def _geocode():
        _require_confirm("run a billable Google Maps geocoding request", confirm)
        if not address:
    ...
    return await run_tool("maps", "geocode", _geocode, allow_retry=False)
...
async def maps_reverse_geocode(
    ...
    def _reverse_geocode():
        _require_confirm("run a billable Google Maps reverse-geocoding request", confirm)
        return _maps_request(
            "GET",
            "https://maps.googleapis.com/maps/api/geocode/json",
    ...
    return await run_tool("maps", "reverse_geocode", _reverse_geocode, allow_retry=False)


@mcp.tool()
async def maps_place_text_search(
    query: str,
    included_type: str = "",
    ...
    def _place_text_search():
        _require_confirm("run a billable Google Maps place search", confirm)
        if not query:
            raise ValueError("query cannot be empty")
        body: dict[str, Any] = {
            "textQuery": query,
    ...
    return await run_tool("maps", "place_text_search", _place_text_search, allow_retry=False)


@mcp.tool()
async def maps_place_details(
    place_id: str,
    field_mask: str = "id,displayName,formattedAddress,location,rating,userRatingCount,websiteUri,nationalPhoneNumber",
    confirm: bool = False,
) -> str:
    """Get Google Places details."""

    def _place_details():
        _require_confirm("run a billable Google Maps place-details request", confirm)
        if not place_id:
    ...
    return await run_tool("maps", "place_details", _place_details, allow_retry=False)


@mcp.tool()
async def maps_compute_routes(
    ...
    def _compute_routes():
        _require_confirm("run a billable Google Maps routing request", confirm)
    ...
    return await run_tool("maps", "compute_routes", _compute_routes, allow_retry=False)


@mcp.tool()
async def merchant_list_products(
    merchant_id: str, max_results: int = 50, page_token: str = ""
) -> str:
...
async def adsense_generate_report(account_name: str, report_request: dict[str, Any]) -> str:
    ...
    def _generate_report():
        ...
        service, cached = client.get_service("adsense", "v2")
        data = (
            service.accounts().reports().generate(parent=account_name, **report_request).execute()
        )
        return data, {"cached_service": cached}
...
async def check_configuration() -> str:
    ...
    def _check_configuration():
        ...
        service_auth_configured = (MCP_MODE == "portal" and len(MCP_PORTAL_GRANT_TOKEN) >= 32) or (
            MCP_MODE == "standalone" and len(MCP_ACCESS_TOKEN) >= 32
        )
        return {
            "configured": service_auth_configured,
            "mode": MCP_MODE,
            "portalGrantRequired": MCP_MODE == "portal",
            "portalGateConfigured": len(MCP_PORTAL_GRANT_TOKEN) >= 32,
            "standaloneAccessTokenConfigured": len(MCP_ACCESS_TOKEN) >= 32,
            ...
            "mapsKeyReadyForCurrentRequest": bool(
                headers.get(MCP_GOOGLE_MAPS_API_KEY_HEADER)
                or (not MCP_DISABLE_DEFAULT_GOOGLE_FALLBACK and GOOGLE_MAPS_API_KEY)
            ),
            "catalogVersion": CATALOG_VERSION,
            "buildSha": _current_tool_manifest()["buildSha"],
        }

    return await run_tool("mcp", "check_configuration", _check_configuration, allow_retry=False)
...
async def list_capabilities(include_descriptors: bool = False) -> str:
    ...
    if not include_descriptors:
        manifest["tools"] = []
    return _bounded_json_dumps(manifest)
...
async def get_endpoint_coverage(
    ...
    def _coverage():
        ...
        for row in ENDPOINT_COVERAGE:
            ...
            normalized_status = "missing" if row_status == "partially_implemented" else row_status
...
async def google_mcp_welcome() -> str:
    ...
    def _welcome():
        return {
            ...
            "access": {
                "mode": MCP_MODE,
                "service_auth_headers": _service_auth_required_headers(),
                "service_auth_configured": (
                    len(MCP_PORTAL_GRANT_TOKEN) >= 32
                    if MCP_MODE == "portal"
                    else len(MCP_ACCESS_TOKEN) >= 32
                ),
            },
            "required_headers": [
                *_service_auth_required_headers(),
                MCP_GOOGLE_CLIENT_ID_HEADER,
                MCP_GOOGLE_CLIENT_SECRET_HEADER,
                MCP_GOOGLE_REFRESH_TOKEN_HEADER,
            ],
...
async def mcp_health_check(
    ...
    """Report privacy-safe local health plus explicitly requested provider checks."""
    ...
    def _health_check():
        ...
        def cache_snapshot() -> dict[str, bool]:
            snapshot = {name: client.is_service_cached(name, version) for name, version in services}
            snapshot["session"] = client.is_session_cached()
            return snapshot
        ...
        health: dict[str, Any] = {
            ...
            "image_reference": MCP_IMAGE_REFERENCE,
            "mode": MCP_MODE,
            "provider_checks_requested": bool(run_checks),
            "provider_warmup_requested": bool(warm_all),
            "provider_credentials_loaded": False,
            "configured_scope_count": len(SCOPES),
            "cache_before": cache_before,
            "cache_after": cache_before,
            "warmup": {},
            "checks": {},
        }
        if not run_checks and not warm_all:
            return health

        creds = client._load_credentials()
        health["provider_credentials_loaded"] = True
        health["credential_valid"] = bool(getattr(creds, "valid", False))
        health["credential_scope_count"] = len(_scope_list(getattr(creds, "scopes", None)))
        health["granted_scope_count"] = len(_scope_list(getattr(creds, "granted_scopes", None)))
        ...
        def record_check(name: str, func, skip_reason: str | None = None) -> None:
            ...
            try:
                cached = bool(func())
                checks[name] = {"ok": True, "cached_service": cached}
            except Exception as exc:
                checks[name] = {"ok": False, "error": _classify_error(exc)}

        warmup: dict[str, Any] = health["warmup"]
        ...
        if warm_all:
            for api_name, api_version in services:
        ...
        if run_checks and not has_scope("drive"):
            ...
            def _drive_check():
                service, cached = client.get_service("drive", "v3")
                service.about().get(fields="kind").execute()
                return cached
        ...
        health["cache_after"] = cache_snapshot()
        return health
...
EXPANDED_TOOL_DESCRIPTIONS = {
    ...
    "maps_geocode": "Use this billable Google Maps Geocoding read only after explicit confirmation. Automatic retry is disabled.",
    "maps_reverse_geocode": "Use this billable Google Maps reverse-geocoding read only after explicit confirmation. Automatic retry is disabled.",
    "maps_place_text_search": "Use this billable Google Maps Places search only after explicit confirmation. Requires a Maps API key and disables automatic retry.",
    "maps_place_details": "Use this billable Google Maps Places details read only after explicit confirmation. Automatic retry is disabled.",
    "maps_compute_routes": "Use this billable Google Maps Routes estimate only after explicit confirmation. Requires a Maps API key and disables automatic retry.",
}
        ...
        [
            ...
            },
            {
                ...
                "missing": [
                    ...
                    "roads",
                    "static maps",
                    "distance matrix migration variants",
                ],
                "tool_refs": [
                    "maps_geocode",
                    "maps_reverse_geocode",
...
def _is_provider_free_request(payload: dict[str, Any] | None) -> bool:
    if not isinstance(payload, dict):
        return False
    ...
    if tool_name in NAVIGATION_TOOLS:
        return True
    if tool_name != "mcp_health_check":
        return False
    arguments = params.get("arguments")
    if not isinstance(arguments, dict):
        arguments = {}
    return arguments.get("run_checks", False) is False and arguments.get("warm_all", False) is False
...
def _valid_build_sha(value: str) -> bool:
    return bool(re.fullmatch(r"[A-Za-z0-9._-]{7,128}", value))
...
def _validate_runtime_configuration() -> None:
    if MCP_MODE not in {"standalone", "portal"}:
        raise RuntimeConfigurationError("MCP_MODE must be either standalone or portal.")
    if not MCP_ALLOWED_HOSTS or any("*" in host for host in MCP_ALLOWED_HOSTS):
        raise RuntimeConfigurationError(
            "MCP_ALLOWED_HOSTS must contain an explicit host allowlist without wildcards."
        )
    if not _valid_build_sha(MCP_BUILD_SHA):
        raise RuntimeConfigurationError("MCP_BUILD_SHA has an invalid format.")
    if not _valid_source_fingerprint(MCP_SOURCE_FINGERPRINT):
        raise RuntimeConfigurationError(
            "MCP_SOURCE_FINGERPRINT must be development or a 64-character hexadecimal digest."
        )
    if not _valid_image_reference(MCP_IMAGE_REFERENCE):
        raise RuntimeConfigurationError(
            "MCP_IMAGE_REFERENCE must be development or an immutable sha256 image reference."
        )
    if MCP_MODE == "portal" and len(MCP_PORTAL_GRANT_TOKEN) < 32:
        raise RuntimeConfigurationError(
            "Portal mode requires MCP_PORTAL_GRANT_TOKEN with at least 32 characters."
        )
    if MCP_MODE == "standalone" and len(MCP_ACCESS_TOKEN) < 32:
        raise RuntimeConfigurationError(
            "Standalone mode requires MCP_ACCESS_TOKEN with at least 32 characters."
        )


def _service_auth_required_headers() -> list[str]:
    if MCP_MODE == "standalone":
        return ["authorization"]
    return [MCP_PORTAL_GRANT_HEADER]


def _validate_service_access(headers: dict[str, str]) -> None:
    if MCP_MODE == "portal":
        if len(MCP_PORTAL_GRANT_TOKEN) < 32:
            raise RuntimeConfigurationError(
                "Server requires a portal grant token with at least 32 characters."
            )
        _validate_portal_grant(headers)
        return
    if MCP_MODE != "standalone":
        raise RuntimeConfigurationError("Server has an invalid MCP_MODE configuration.")
    if len(MCP_ACCESS_TOKEN) < 32:
        raise RuntimeConfigurationError(
            "Server requires a standalone access token with at least 32 characters."
        )
    authorization = headers.get("authorization", "")
    scheme, separator, supplied = authorization.partition(" ")
    if not separator or scheme.lower() != "bearer" or not supplied.strip():
        raise ValueError("Missing required Authorization Bearer token.")
    if not hmac.compare_digest(supplied.strip(), MCP_ACCESS_TOKEN):
        raise ValueError("Invalid Authorization Bearer token.")
...
def _validate_request_host(headers: dict[str, str]) -> None:
    supplied = headers.get("host", "")
    if not supplied:
        raise ValueError("Missing required Host header.")
    normalized = _normalized_request_host(supplied)
    allowed = {_normalized_request_host(host) for host in MCP_ALLOWED_HOSTS}
    if normalized not in allowed and supplied.lower() not in MCP_ALLOWED_HOSTS:
        raise ValueError("Request Host is not trusted.")


def _validate_request_origin(headers: dict[str, str]) -> None:
    origin = headers.get("origin", "").strip()
    if not origin:
        return
    if not MCP_ALLOWED_ORIGINS or origin not in MCP_ALLOWED_ORIGINS:
        raise ValueError("Request Origin is not allowed.")


def _validate_tool_confirmation(payload: dict[str, Any] | None) -> None:
    ...
    properties = getattr(tool, "parameters", {}).get("properties", {}) if tool else {}
    if "confirm" not in properties:
        return
    arguments = params.get("arguments")
    if not isinstance(arguments, dict) or arguments.get("confirm") is not True:
        raise ValueError(f"confirm=true is required to call {tool_name}.")


FANOUT_ARGUMENT_BATCH_SIZES = {
    "drive_batch_get_metadata": ("file_ids", 1),
    "gmail_batch_get_metadata": ("message_ids", 1),
    "gmail_batch_modify_messages": ("message_ids", 1000),
    "gmail_batch_delete_messages": ("message_ids", 1000),
    "calendar_batch_get_events": ("event_ids", 1),
}


def _validate_tool_execution_limits(payload: dict[str, Any] | None) -> None:
    if not isinstance(payload, dict) or payload.get("method") != "tools/call":
        return
    params = payload.get("params")
    if not isinstance(params, dict):
    ...
    for argument_name, value in arguments.items():
        if isinstance(value, list) and len(value) > MCP_MAX_BATCH_ITEMS:
            raise ValueError(
                f"{argument_name} exceeds the {MCP_MAX_BATCH_ITEMS}-item batch limit. "
                "Split the work into smaller requests."
            )

    fanout = FANOUT_ARGUMENT_BATCH_SIZES.get(tool_name)
    if fanout:
        argument_name, items_per_provider_call = fanout
        value = arguments.get(argument_name)
        if isinstance(value, list):
            _validate_fanout_items(
                value,
                argument_name,
                items_per_provider_call=items_per_provider_call,
            )
    elif tool_name == "gmail_apply_cleanup_plan":
        ...
        if isinstance(value, list):
            ...
            _validate_fanout_items(
                value,
                "message_ids",
                items_per_provider_call=1 if action == "trash" else 1000,
            )
    elif tool_name in {"gmail_sender_clusters", "gmail_cleanup_plan"}:
        try:
            max_messages = int(arguments.get("max_messages", 500))
            page_size = int(arguments.get("page_size", 100))
        except (TypeError, ValueError) as exc:
            raise ValueError("max_messages and page_size must be integers.") from exc
        _validate_gmail_scan_fanout(
            max_messages,
            page_size,
        )
...
def _validate_portal_grant(headers: dict[str, str]) -> None:
    if len(MCP_PORTAL_GRANT_TOKEN) < 32:
        raise ValueError("Server requires a portal grant token with at least 32 characters.")
...
def _has_canonical_argument(arguments: dict[str, Any], canonical_key: str) -> bool:
    return canonical_key in arguments or any(
        isinstance(key, str) and _camel_to_snake(key) == canonical_key for key in arguments
    )
...
async def _read_request_body(
    receive,
    max_bytes: int = MCP_REQUEST_BODY_MAX_BYTES,
) -> bytes:
    chunks: list[bytes] = []
    total_bytes = 0
    while True:
        ...
        if body:
            ...
            if total_bytes > max_bytes:
                raise RequestBodyTooLarge(f"Request body exceeds the {max_bytes}-byte limit.")
            chunks.append(body)
...
def build_hosted_mcp_http_wrapper(app):
    async def _wrapped(scope, receive, send):
        ...
        try:
            _validate_request_host(normalized_headers)
        except ValueError as exc:
            await _send_json(
                send,
                400,
                {"ok": False, "error": "invalid_host", "message": str(exc)},
                extra_headers=[(b"cache-control", b"no-store")],
            )
            return
        try:
            _validate_request_origin(normalized_headers)
        except ValueError as exc:
            await _send_json(
                send,
                403,
                {"ok": False, "error": "origin_not_allowed", "message": str(exc)},
                extra_headers=[(b"cache-control", b"no-store")],
            )
            return
        ...
        if path == "/health":
            ...
            service_auth_configured = (
                MCP_MODE == "portal" and len(MCP_PORTAL_GRANT_TOKEN) >= 32
            ) or (MCP_MODE == "standalone" and len(MCP_ACCESS_TOKEN) >= 32)
            trusted_hosts_configured = bool(MCP_ALLOWED_HOSTS) and not any(
                "*" in host for host in MCP_ALLOWED_HOSTS
            )
            provenance_valid = (
                _valid_build_sha(MCP_BUILD_SHA)
                and _valid_source_fingerprint(MCP_SOURCE_FINGERPRINT)
                and _valid_image_reference(MCP_IMAGE_REFERENCE)
            )
            configuration_ready = (
                service_auth_configured
                and trusted_hosts_configured
                and provenance_valid
                and MCP_ALLOW_REQUEST_OVERRIDES
            )
            await _send_json(
                send,
                200,
                {
                    ...
                    "configuration": {
                        ...
                        "portal_grant_configured": len(MCP_PORTAL_GRANT_TOKEN) >= 32,
                        "provider_credentials_mode": "per_request_byok",
                        "default_provider_file_fallback_enabled": not MCP_DISABLE_DEFAULT_GOOGLE_FALLBACK,
                        "byok_client_cache_enabled": bool(
                            MCP_BYOK_CLIENT_CACHE_SIZE and MCP_BYOK_CLIENT_CACHE_TTL_SECONDS
                        ),
        ...
        try:
            _validate_service_access(normalized_headers)
            ...
            await _send_jsonrpc_error(
                ...
                message=str(exc),
                request_id="server-error",
                data={"required_headers": _service_auth_required_headers()},
            )
            return
        ...
        body = b""
        consumed_body = False
        provider_free_request = method in {"GET", "DELETE"}
        if method in {"POST", "PUT", "PATCH"}:
            ...
            if content_length:
                try:
                    ...
                    await _send_jsonrpc_error(
                        ...
                        status=400,
                        code=-32600,
                        message="Bad Request: Content-Length must be an integer",
                        request_id=jsonrpc_id,
                    )
                    return
                if declared_length > MCP_REQUEST_BODY_MAX_BYTES:
                    await _send_jsonrpc_error(
                        send,
                        status=413,
                        code=-32003,
            ...
            try:
                _validate_tool_confirmation(payload)
                _validate_tool_execution_limits(payload)
            except ValueError as exc:
                await _send_jsonrpc_error(
                    send,
                    status=400,
                ...
                return
            provider_free_request = _is_provider_free_request(payload)
            body = json.dumps(payload, ensure_ascii=True, separators=(",", ":")).encode("utf-8")
...
def build_app():
    """Build the hardened ASGI application for Uvicorn or embedding."""
...
def main() -> None:
    """Run the hosted Google MCP server."""
