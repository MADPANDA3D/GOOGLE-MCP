...
SERVICE_ID = "google"
CATALOG_VERSION = "google-2026.07.18.2"
...
BILLABLE_CONFIRMED_READ_TOOLS = frozenset(
    {
        "maps_geocode",
...
def _annotations(tool: Any) -> dict[str, bool]:
    ...
    values = source.model_dump(by_alias=True, exclude_none=True) if source is not None else {}
    return {
...
def _complete_description(
    ...
    *,
    ...
    if name in STANDARD_NAVIGATION_TOOLS:
        suffix = (
            " This is a local, read-only navigation operation. Transport authentication "
            "follows the configured runtime mode; it does not contact Google or require "
            "Google OAuth credentials."
        )
    elif name in BILLABLE_CONFIRMED_READ_TOOLS:
        suffix = (
            " This is a billable, confirmed Google Maps read: it does not mutate or "
            "delete Google data. Use it only after preview and explicit confirmation; "
            "automatic retry is disabled to avoid duplicate quota or billing consumption."
        )
    elif risk == "read":
        suffix = (
        ...
        suffix = (
            ...
            "Use it only after preview and the manifest-defined explicit confirmation."
        )
    else:
        suffix = (
            " It creates or updates Google provider state and requires per-request Google OAuth. "
...
def _confirmation(
    ...
    requires_confirmation = annotations["destructiveHint"] or "confirm" in properties
    ...
    return {
        ...
        "exactPhrase": phrase,
        "when": (
            "For a direct tool call, set confirm=true only after preview and explicit "
            "approval. Through the MAD MCP Portal, supply this exact phrase to "
            "portal.call_destructive_tool after preview; the phrase is not a provider "
            "credential or native argument."
        ),
    }
...
def build_tool_manifest(registered_tools: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if missing_navigation:
        raise RuntimeError(
            f"Google ToolManifest drift; missing navigation tools={sorted(missing_navigation)}."
        )
    ...
    for name in sorted(names):
        ...
        confirmation = _confirmation(name, annotations, input_schema)
        descriptor: dict[str, Any] = {
            ...
            "deprecation": copy.deepcopy(DEPRECATIONS.get(name, DEFAULT_DEPRECATION)),
...
def search_manifest(
    ...
    *,
    ...
    for descriptor in manifest.get("tools", []):
        ...
        alias_text = " ".join(
            " ".join(normalize_search_tokens(alias)) for alias in descriptor.get("aliases", [])
        )
        ...
        haystack = " ".join((normalized_name, alias_text, title, category_text, description))
